...
@endif
    <div class="w-full min-h-screen flex justify-center items-center bg-green-100">
        <div class="relative w-fit h-fit bg-white p-8 rounded-xl">
            ...
            <p class="block text-center">login sebagai administrator</p>
            <form action="{{route('auth.login_post',['as'=>'admin'])}}" method="POST">
                @csrf
                <livewire:components.macro.forms.input-username :key="'username'" addClassMain="block mt-2" />
